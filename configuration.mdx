---
title: "Configure Datamailer with Environment Variables"
sidebarTitle: "Configuration"
description: "Complete reference for all Datamailer environment variables covering Django settings, database, AWS SES, SQS queues, and sending defaults."
---
...
<ParamField path="PUBLIC_BASE_URL" type="string" default="http://localhost:8000">
  The externally reachable base URL of your Datamailer deployment, without a trailing slash. Used to construct open-tracking pixel URLs, click-tracking redirect URLs, and unsubscribe links that are embedded in outgoing emails.
...
<Note>
  The in-app interactive API docs at `/api-docs/` use `DATAMAILER_API_DOCS_BASE_URL` as the base URL for copy-pasteable `curl` examples. This lets staff run the examples against a different environment — for example, a staging server — without changing the server's own `PUBLIC_BASE_URL`. If not set, it falls back to `PUBLIC_BASE_URL`.

  Set `DATAMAILER_API_DOCS_BASE_URL` when you want the in-app examples to target a different host than the one serving the UI.
